...
class GrabExpressClient
{
    ...
    {
        $this->baseUrl = config('services.grabexpress.base_url');
        $this->apiKey = config('services.grabexpress.api_key');
        $this->quotePath = (string) config('services.grabexpress.quote_path', '/quotes');
        $this->timeoutSeconds = (int) config('services.grabexpress.timeout_seconds', 15);
    }
    ...
    public function quote(array $origin, array $destination, array $payload = []): OnDemandQuoteResult
    {
        if (!$this->isConfigured()) {
            return new OnDemandQuoteResult(false, null, null, null, null, [], 'GrabExpress is not configured');
        }
        ...
        if (!$response->successful()) {
            return new OnDemandQuoteResult(false, null, null, null, null, $response->json() ?? [], 'Failed to get GrabExpress quote');
        }
        ...
        if ($price === null) {
            return new OnDemandQuoteResult(false, null, $currency, $serviceName, $etd, $json, 'GrabExpress quote response missing price');
        }
